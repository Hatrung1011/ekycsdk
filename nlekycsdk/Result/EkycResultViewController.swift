//
//  EkycResultViewController.swift
//  nlekycsdk
//
...
class EkycResultViewController: UIViewController {
    ...
    var combinedCallbackData: String?
    ...
    override func viewDidLoad() {
        ...
        let verifyDateAttrs = NSMutableAttributedString(
            string: "Thời gian xác thực: ",
            attributes: [
                .foregroundColor: UIColor.frameworkColor(named: "Neutral Color900")!,
                .font: UIFont.systemFont(ofSize: 13),
            ])
        verifyDateAttrs.append(
            NSAttributedString(
                string: Date().string(withFormat: "HH:mm - dd/MM/yyyy"),
                attributes: [
                    .foregroundColor: UIColor.frameworkColor(named: "Neutral Color700")!,
                    .font: UIFont.systemFont(ofSize: 13),
                ]))
    ...
    func setupCardData() {
        ...
        if let citizen_identify = credentialSharing?.citizenIdentify {
            self.items?.append(
                UITableViewCell.CellConfiguration(
                    text: "CCCD",
    ...
    @IBAction func onPressReturnHome(_ sender: Any) {
        // Call callback with combined data if available, otherwise with card information
        if let combinedData = self.combinedCallbackData {
            // Use the combined data (liveness + card info)
            self.ekycCallback?(combinedData)
        } else if let cardInfo = self.cardInfo {
            // Fallback to card info only
            let jsonString = IDCardJSONConverter.convertToJSONString(cardInfo)
            self.ekycCallback?(jsonString)
        } else {
            self.ekycCallback?("No data available")
        }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell =
            tableView.dequeueReusableCell(
                withIdentifier: ShowCardInfoTextTableViewCell.identifier, for: indexPath)
            as! ShowCardInfoTextTableViewCell
