...
class CardManager: NSObject {
    ...
    static var cardInformation: CardInformation? {
        get {
            ...
            if keychain.getData("kCardInformation") != nil {
                do {
    ...
    static func getAdditionalSharingProperties() -> [String] {
        let properties: [String] = [
            "personalIdentification", "dateProvide",
            //                                               "Nơi cấp",
            "dateOfExpiry",
            //                                               "Địa chỉ hiện tại",
            "fatherName",
...
extension CardManager {
    ...
    static func createPublicKey(pem: String) -> SecKey? {
        ...
        let pemWithoutHeaderFooterNewlines = base64EncodedString.trimmingCharacters(
    ...
    static func createSecKeyFromPrivateKey(privateKeyPEM: String) -> SecKey? {
        ...
        guard
            let keyData = Data(base64Encoded: base64EncodedData, options: .ignoreUnknownCharacters)
        else {
            return nil
        }
        ...
        guard
            let privateKey = SecKeyCreateWithData(
                test as CFData, attributes as CFDictionary, &error)
        else {
            debugPrint(error!)
            return nil
        }
