...
class ERTextField: UITextField {
    ...
    func initCommon() {
        ...
        self.layer.borderColor =
            UIColor.frameworkColor(named: "Neutral Color300")?.cgColor ?? UIColor.lightGray.cgColor
    ...
    func showAsDropDownField(data: [String]) {
        ...
        view.addGestureRecognizer(
            UITapGestureRecognizer(target: self, action: #selector(handleDropDownAction)))
    ...
    func showAsSecurityField() {
        ...
        rightView.addGestureRecognizer(
            UITapGestureRecognizer(target: self, action: #selector(toggleSecurityTextField)))
    ...
    @objc func toggleSecurityTextField() {
        ...
        let color = UIColor.frameworkColor(named: "Neutral Color400") ?? UIColor.gray
...
extension UIView {
    class func paddingViewWithFontawesomeImage(
        image: String, padding: CGFloat = 15, imageSize: CGFloat = 20
    ) -> UIView {
        let color = UIColor.lightGray

        let img =
            UIImage(
                icon: image, backgroundColor: UIColor.clear, iconColor: color,
                andSize: CGSize(width: 20, height: 20)) ?? UIImage()
    ...
    class func paddingViewWithSystemImage(
        image: String, padding: CGFloat = 15, imageSize: CGFloat = 20
    ) -> UIView {
        let color = UIColor.frameworkColor(named: "Neutral Color400") ?? UIColor.lightGray
