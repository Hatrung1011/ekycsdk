...
public class QKMRZScannerView: UIView {
    fileprivate lazy var tesseract: SwiftyTesseract = {
        ...
        guard let bundle = frameworkBundle else {
            fatalError("Could not find framework bundle for SwiftyTesseract")
        }

        return SwiftyTesseract(
            language: .custom("ocrb"), dataSource: bundle, engineMode: .tesseractOnly)
    }()
    ...
    fileprivate func cutoutRect(for cgImage: CGImage) -> CGRect {
        ...
        if videoOrientation == .portrait || videoOrientation == .portraitUpsideDown {
            return CGRect(
                x: (rect.minY * imageWidth), y: (rect.minX * imageHeight),
                width: (rect.height * imageWidth), height: (rect.width * imageHeight))
        } else {
            return CGRect(
                x: (rect.minX * imageWidth), y: (rect.minY * imageHeight),
    ...
    fileprivate func enlargedDocumentImage(from cgImage: CGImage) -> UIImage {
        ...
        croppingRect = CGRect(
            x: (croppingRect.minX - margin), y: (croppingRect.minY - margin),
            width: croppingRect.width + (margin * 2), height: croppingRect.height + (margin * 2))
    ...
    fileprivate func initCaptureSession() {
        ...
        //            }
        //        }
        observer = captureSession.observe(\.isRunning, options: [.new]) {
        ...
        if captureSession.canAddInput(deviceInput) && captureSession.canAddOutput(videoOutput) {
            ...
            videoOutput.setSampleBufferDelegate(
                self,
                queue: DispatchQueue(
                    label: "video_frames_queue", qos: .userInteractive, attributes: [],
                    autoreleaseFrequency: .workItem))
            videoOutput.alwaysDiscardsLateVideoFrames = true
            videoOutput.videoSettings =
                [kCVPixelBufferPixelFormatTypeKey: kCVPixelFormatType_32BGRA] as [String: Any]
            videoOutput.connection(with: .video)!.videoOrientation = AVCaptureVideoOrientation(
    ...
    fileprivate func adjustVideoPreviewLayerFrame() {
        videoOutput.connection(with: .video)?.videoOrientation = AVCaptureVideoOrientation(
            orientation: interfaceOrientation)
        videoPreviewLayer.connection?.videoOrientation = AVCaptureVideoOrientation(
            orientation: interfaceOrientation)
    ...
    fileprivate func preprocessImage(_ image: CGImage) -> CGImage {
        ...
        inputImage = inputImage.applyingFilter(
            "CIExposureAdjust", parameters: ["inputEV": exposure]
        )
...
extension QKMRZScannerView: AVCaptureVideoDataOutputSampleBufferDelegate {
    public func captureOutput(
        ...
        let detectTextRectangles = VNDetectTextRectanglesRequest { [unowned self] request, error in
            ...
            let transform = CGAffineTransform.identity.scaledBy(x: imageWidth, y: -imageHeight)
                .translatedBy(x: 0, y: -1)
            let mrzTextRectangles = results.map({ $0.boundingBox.applying(transform) }).filter({
                $0.width > (imageWidth * 0.6)
            })
                ...
                if let mrzResult = self.mrz(from: mrzTextImage), mrzResult.allCheckDigitsValid {
                    ...
                    DispatchQueue.main.async {
                        ...
                        let scanResult = QKMRZScanResult(
                            mrzResult: mrzResult, documentImage: enlargedDocumentImage)
...
extension AVCaptureDevice {
    ...
    static var bestForVideo: AVCaptureDevice? {
        var deviceHasUltraWideCamera = !AVCaptureDevice.DiscoverySession(
