...
class LivenessVC: UIViewController, LivenessUtilityDetectorDelegate {
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        Task {
            do {
                try livenessDetector?.getVerificationRequiresAndStartSession(
    ...
    func liveness(
        liveness: LivenessUtilityDetector, didFinish verificationImage: UIImage,
        livenesScore: Float, faceMatchingScore: Float, result: Bool, message: String,
        videoURL: URL?, response: LivenessResult?
    ) {
